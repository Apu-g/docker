...
def main():
    # Step 3C: Merge AST and semantic JSONs
    print("Step 3C: Merging AST and semantic data...")
    ast_path = Path('graphify-out/.graphify_ast.json')
    sem_path = Path('graphify-out/.graphify_semantic.json')
    
    if not ast_path.exists():
        print(f"Error: {ast_path} not found.")
        sys.exit(1)
    if not sem_path.exists():
        print(f"Error: {sem_path} not found.")
        sys.exit(1)
    ...
    print("Step 4: Building graph and clustering communities...")
    ...
    report = generate(G, communities, cohesion, labels, gods, surprises, detection, tokens, 'c:\\Users\\Apu Ghanti\\Desktop\\docker', suggested_questions=questions)
